...
bool QCP::gIsLatexMarkupEnabled;
//int QCP::gLegendFrameCornerRadius;
//bool QCP::gIsLegendFrameShadowEnabled;
...
{
  mSize = size;
  if (mShape == QCPScatterStyle::ssPixmap) {
      setPixmap(mIcon.pixmap(mSize, mSize));
  }
}
...
void QCPScatterStyle::setIcon(const QIcon &icon)
{
    mIcon = icon;
    setPixmap(mIcon.pixmap(mSize, mSize));
}
...
void QCPScatterStyle::setDecimation(int decimation)
{
...
void QCPScatterStyle::drawShape(QCPPainter *painter, double x, double y) const
{
  ...
  switch (mShape)
  {
    ...
    case ssTriangle:
    {
       ...
       //SAE
//       QPoint pointVector[3] = {QPoint(x-w, y+0.755*w), QPoint(x+w, y+0.755*w), QPoint(x, y-0.977*w)};
//       painter->drawPolygon(pointVector, 3);
       //SAE
  ...
  if (mDataLabelsEnabled) {
      QFontMetrics fontMetrics(painter->font());
      QString dataLabel = valueToString(y, GraphModel::StringRepresentation::Decimal);
      QPen oldPen = painter->pen();
      QPen newPen = oldPen;
      newPen.setColor(mDataLabelsFontColor);
      painter->setPen(newPen);
      painter->drawText(x - fontMetrics.width(dataLabel) / 2, y +  fontMetrics.height()/2 + mSize/2 + 8, dataLabel);
...
void QCPGrid::drawGridLines(QCPPainter *painter) const
{
  ...
  if (mParentAxis->orientation() == Qt::Horizontal)
  {
    ...
    painter->setPen(mPen);
    //SAE
    //NOTE: to fix bug with gray rects.
//    QPen subPen = mPen;
//    subPen.setCapStyle(Qt::RoundCap);
//    painter->setPen(subPen);
    ...
    for (int i=lowTick; i <= highTick; ++i)
    {
      ...
      t = mParentAxis->coordToPixel(mParentAxis->mTickVector.at(i)); // x
      painter->drawLine(QLineF(t, mParentAxis->mAxisRect->bottom(), t, mParentAxis->mAxisRect->top()));
      //NOTE: I don't know why but the line above has a bug - sometimes it draws rectangulars. The line below doesn't.
      //Also bug may be eliminated by setting cap style (see commented code above).
      //SAE
//      painter->drawLine(QLineF(t, mParentAxis->mAxisRect->bottom(), t, mParentAxis->mAxisRect->top()+1.));
      //SAE
    }
  } else
  {
...
//      painter->drawLine(QLineF(t, mParentAxis->mAxisRect->bottom(), t, mParentAxis->mAxisRect->top()));
      ...
      //Also bug may be eliminated by setting cap style (see commented code above).
      //SAE
      painter->drawLine(QLineF(t, mParentAxis->mAxisRect->bottom(), t, mParentAxis->mAxisRect->top()-1.0));
      //SAE
    }
  } else
  {
...
QCPAxis::QCPAxis(QCPAxisRect *parent, AxisType type) :
  ...
  mCachedMargin(0),
  mTickEvalutaionPolicy(preserveSpacing),
  mLabelDeviationForLeftAxis(dontMove),
  mIsEdgeTicksModificationEnabled(false)
...
void QCPAxis::setTickEvaluationPolicy(QCPAxis::TickEvaluationPolicy policy)
{
    mTickEvalutaionPolicy = policy;
}

void QCPAxis::setLabelDeviationForLeftAxis(QCPAxis::LabelDeviationForLeftAxis deviation)
{
    mLabelDeviationForLeftAxis = deviation;
...
void QCPAxis::setupTickVectors()
{
  ...
  if (mIsEdgeTicksModificationEnabled == false) { //SAE
  ...
  //SAE
      ...
      QVector<double> pseudoTickVector = mTickVector;

      if (pseudoTickVector.size() > 3) {
          pseudoTickVector[0] = pseudoTickVector[1] - (pseudoTickVector[2] - pseudoTickVector[1]);
          pseudoTickVector.push_front(pseudoTickVector[1] - (pseudoTickVector[2] - pseudoTickVector[1]));
          pseudoTickVector.push_front(pseudoTickVector[1] - (pseudoTickVector[2] - pseudoTickVector[1]));
          pseudoTickVector[pseudoTickVector.size() - 1] =
                  pseudoTickVector[pseudoTickVector.size() - 2]
                  + (pseudoTickVector[pseudoTickVector.size() - 2] - pseudoTickVector[pseudoTickVector.size() - 3]);
          pseudoTickVector.push_back(
                                pseudoTickVector[pseudoTickVector.size() - 2]
                                + (pseudoTickVector[pseudoTickVector.size() - 2] - pseudoTickVector[pseudoTickVector.size() - 3])
                                    );
          pseudoTickVector.push_back(
      ...
      mSubTickVector.resize((pseudoTickVector.size()-1)*mSubTickCount);
      if (mSubTickCount > 0)
      {
        double subTickStep = 0;
        double subTickPosition = 0;
    ...
    //    int lowTick = mLowestVisibleTick > 0 ? mLowestVisibleTick-1 : mLowestVisibleTick;
    //    int highTick = mHighestVisibleTick < pseudoTickVector.size()-1 ? mHighestVisibleTick+1 : mHighestVisibleTick;

    //    for (int i=lowTick+1; i<=highTick; ++i)  //original
    //    for (int i=1; i<=highTick+2; ++i)
        for (int i=1; i<pseudoTickVector.size(); ++i)
        {
          subTickStep = (pseudoTickVector.at(i)-pseudoTickVector.at(i-1))/(double)(mSubTickCount+1);
          for (int k=1; k<=mSubTickCount; ++k)
          {
            subTickPosition = pseudoTickVector.at(i-1) + k*subTickStep;
            if (subTickPosition < mRange.lower)
              continue;
            if (subTickPosition > mRange.upper)
            {
...
void QCPAxis::generateAutoTicks()
{
  if (mScaleType == stLinear)
  {
    if (mAutoTickStep)
    {
      ...
      //SAE
      if (mTickEvalutaionPolicy == preserveSpacing) {
                if (tickStepMantissa < 5)
                {
                  int x = (int)(tickStepMantissa*2);
                  if (x == 9) {
                      x = 8;
                  } else if (x == 7){
                      x = 8;
                  }
                  mTickStep = x/2.0*magnitudeFactor;
                } else
                {
                  mTickStep = (int)(tickStepMantissa/2.0)*2.0*magnitudeFactor;
                }
...
//            break;
    ...
    //SAE
    if (mIsEdgeTicksModificationEnabled) {
        double  xMin = mRange.center() - 0.5*mRange.size();
        double  xMax = mRange.center() + 0.5*mRange.size();

        while (mTickVector[0] < xMin) {
            mTickVector.remove(0);
            tickcount--;
        }
        while (mTickVector[tickcount-1] > xMax) {
            mTickVector.remove(tickcount-1);
            tickcount--;
        }

        if ((xMax - mTickVector[tickcount-1]) < 0.6*mTickStep)
            mTickVector[tickcount-1] = xMax;
        else {
            mTickVector.push_back(xMax);
            tickcount++;
        }
        if ((mTickVector[0] - xMin) < 0.6*mTickStep)
            mTickVector[0] = xMin;
        else {
            mTickVector.push_front(xMin);
            tickcount++;
        }

        //adding 0 tick if necessary
        if (mAxisType==atLeft && mTickVector[0]*mTickVector.back() < 0 && mTickVector.indexOf(0.0) == -1) {
            double max = std::max(std::abs(mTickVector[0]), std::abs(mTickVector.back()));
            double min = std::min(std::abs(mTickVector[0]), std::abs(mTickVector.back()));
            //magic condition :)
            if (min > max*0.15) {
//                auto it = qLowerBound(mTickVector.begin(), mTickVector.end());
                mTickVector.insert(qLowerBound(mTickVector.begin(), mTickVector.end(), 0.), 0.);
            }
        }
    }
    //SAE
...
//  const int highTick = mHighestVisibleTick;
  ...
  //SAE 19.07.16 //some ticks at the end dissappear
  //const int lowTick = 0;
  //const int highTick = mTickVector.size()-1;
  //SAE

  //SAE
  const int lowTick  = mIsEdgeTicksModificationEnabled ?                    0 : mLowestVisibleTick;
  const int highTick = mIsEdgeTicksModificationEnabled ? mTickVector.size()-1 : mHighestVisibleTick;
  //SAE
  ...
  //SAE
  if (mAxisType == AxisType::atBottom) {
      if (mAxisRect->isXAxisDrawingAtCenterEnabled()) {
          auto yRange = mAxisRect->axis(AxisType::atLeft)->range();
          if (yRange.lower * yRange.upper < 0) {
              mAxisPainter->offset = -mAxisRect->rect().height()*qAbs(yRange.lower/(yRange.upper - yRange.lower));
              mAxisPainter->labelPadding = -mAxisPainter->offset - QFontMetrics(mAxisPainter->tickLabelFont).height();
          }
      }
  }
  if (mAxisType == AxisType::atLeft) {
      if (mAxisRect->isYAxisDrawingAtCenterEnabled()) {
          auto xRange = mAxisRect->axis(AxisType::atBottom)->range();
          if (xRange.lower * xRange.upper < 0) {
              mAxisPainter->offset = -mAxisRect->rect().width()*qAbs(xRange.lower/(xRange.upper - xRange.lower));
              //hard to count this correction
              mAxisPainter->labelPadding = -mAxisPainter->offset; ///*hard to count this correction*/ - QFontMetrics(mAxisPainter->tickLabelFont).height();
          }
      }
  }

  mAxisPainter->labelDeviationForLeftAxis = mLabelDeviationForLeftAxis;
  //SAE
...
int QCPAxis::calculateMargin()
{
  ...
  //SAE
  //to remove tick label font from consideration if x axis should be drawn at center
  if (mAxisType == AxisType::atBottom) {
      if (mAxisRect->isXAxisDrawingAtCenterEnabled()) {
          auto yRange = mAxisRect->axis(AxisType::atLeft)->range();
          if (yRange.lower * yRange.upper < 0) {
              QFont dummyFont;
              dummyFont.setPointSize(0.1);
              mAxisPainter->tickLabelFont = dummyFont;
          }
      }
  }
  //SAE
...
void QCPAxisPainterPrivate::draw(QCPPainter *painter)
{
  ...
  //SAE
  bool oldAntial = painter->antialiasing();
  painter->setAntialiasing(true);
  //SAE
  ...
  //SAE
  painter->setAntialiasing(oldAntial);
  //SAE
...
//        tickPositions[0] -= painter->font().pointSize();
//        tickPositions[tickPositions.size()-1] += painter->font().pointSize();


          enum LabelDeviationForLeftAxis {
                                        moveTopAndBotton,
                                        moveOnlyTop,
                                        moveOnlyBotton,
                                        dontMove
          ...
          if (labelDeviationForLeftAxis == QCPAxis::moveTopAndBotton || labelDeviationForLeftAxis == QCPAxis::moveOnlyTop)
            tickPositions[tickPositions.size()-1] += tickLabelFont.pointSize() / 2;
          if (labelDeviationForLeftAxis == QCPAxis::moveTopAndBotton || labelDeviationForLeftAxis == QCPAxis::moveOnlyBottom)
            tickPositions[0] -= tickLabelFont.pointSize() / 2;
...
//          };
    ...
    case QCPAxis::atTop:
      //debug //SAE
//      qDebug() << "tickPositions attop" <<tickPositions;
      //debug //SAE
        break;
    default:
      break;
...
//      painter->drawText(0, 0, axisRect.height(), labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter, label);
      //SAE
      if (QCP::gIsLatexMarkupEnabled) {
          QString newText;
          bool replacementsTookPlace;
          std::tie(newText, replacementsTookPlace) = latex2html(label);
          if (replacementsTookPlace) {
              QTextDocument td;
//              td.setTextWidth( mTextBoundingRect.width() );
              td.setHtml(newText);
              drawHtmlText(painter, QRect(0, 0, axisRect.height(), labelBounds.height()), Qt::TextDontClip | Qt::AlignCenter, td);
          } else {
              painter->drawText(0, 0, axisRect.height(), labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter, label);
          }
      } else {
      ...
      painter->setTransform(oldTransform);
        ...
        //SAE
//      painter->drawText(5/*0*/, origin.y()-axisRect.height(), labelBounds.width(), axisRect.height(), Qt::TextDontClip | Qt::AlignVCenter | Qt::AlignLeft, label);
      //SAE
    }
...
//              td.setTextWidth( mTextBoundingRect.width() );
              td.setHtml(newText);
              drawHtmlText(painter, QRect(origin.x(), origin.y()+margin, axisRect.width(), labelBounds.height()), Qt::TextDontClip | Qt::AlignCenter, td);
          } else {
              painter->drawText(origin.x(), origin.y()+margin, axisRect.width(), labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter, label);
          }
      } else {
...
double QCPAbstractItem::rectSelectTest(const QRectF &rect, const QPointF &pos, bool filledRect) const
{
  ...
  if (filledRect && result > mParentPlot->selectionTolerance()*0.99)
  {
    ...
    //SAE  to allow easy arrowed text capturing
//    if (rect.contains(pos))
//        result = 0.0001;
    //SAE
  }
...
void QCustomPlot::draw(QCPPainter *painter)
{
  ...
  //SAE
//  QVector<QCPLayerable *> graphVector;
//  QVector<QCPLayerable *> axisVector;
//  QVector<QCPLayerable *> tracerVector;
////  QVector<QCPLayerable *> legendsVector;

//  QVector<QCPLayerable *> itemsVector;
//  foreach (QCPLayer *layer, mLayers)
//  {
//    foreach (QCPLayerable *child, layer->children())
//    {
//      if (child->realVisibility())
//      {
//        if (dynamic_cast<QCPGraph*>(child))
//            graphVector.push_back(child);
//        else if (dynamic_cast<QCPAxis*>(child)) {
//            axisVector.push_back(child);
//        } else if (dynamic_cast<QCPItemTracer*>(child)) {
//            tracerVector.push_back(child);
//        } else if (dynamic_cast<QCPAbstractItem*>(child)) {
//            itemsVector.push_back(child);
//        }/* else if (dynamic_cast<QCPLegend*>(child)) {
//            legendsVector.push_back(child);
//        }*/ else {
//            painter->save();
//            painter->setClipRect(child->clipRect().translated(0, -1));
//            child->applyDefaultAntialiasingHint(painter);
...
//  foreach (QCPLayerable *child, axisVector) {
//      painter->save();
//      painter->setClipRect(child->clipRect().translated(0, -1));
//      child->applyDefaultAntialiasingHint(painter);
//      child->draw(painter);
...
//  foreach (QCPLayerable *child, graphVector) {
//      painter->save();
//      painter->setClipRect(child->clipRect().translated(0, -1));
//      child->applyDefaultAntialiasingHint(painter);
//      child->draw(painter);
...
//  foreach (QCPLayerable *child, tracerVector) {
//      painter->save();
//      painter->setClipRect(child->clipRect().translated(0, -1));
//      child->applyDefaultAntialiasingHint(painter);
//      child->draw(painter);
...
//  foreach (QCPLayerable *child, itemsVector) {
//      painter->save();
//      painter->setClipRect(child->clipRect().translated(0, -1));
//      child->applyDefaultAntialiasingHint(painter);
//      child->draw(painter);
...
//  foreach (QCPLayerable *child, legendsVector) {
//      painter->save();
//      painter->setClipRect(child->clipRect().translated(0, -1));
//      child->applyDefaultAntialiasingHint(painter);
//      child->draw(painter);
...
//        QString newText;
//        bool replacementsTookPlace;
//        std::tie(newText, replacementsTookPlace) = latex2html(mPlottable->name());
//        if (replacementsTookPlace) {
//            QTextDocument td;
//  //              td.setTextWidth( mTextBoundingRect.width() );
//            td.setHtml(newText);
//            textRect = QRectF(QPoint(0,0), sizeHint(painter, Qt::TextDontClip, td));
//        }
//    }
    //SAE
...
//      bool replacementsTookPlace;
//      std::tie(newText, replacementsTookPlace) = latex2html(mPlottable->name());
//      //TODO: determine why drawHtmlText indent text and them uncomment treatment for simple drawText
//      if (replacementsTookPlace) {
//          QTextDocument td;
////          td.setTextWidth( mTextBoundingRect.width() );
//          td.setHtml(newText);
//          drawHtmlText(painter, QRect(mRect.x()+iconSize.width()+mParentLegend->iconTextPadding(), mRect.y(), textRect.width(), textHeight), Qt::TextDontClip | Qt::AlignVCenter | Qt::AlignLeft, td);
//      } else {
//          painter->drawText(mRect.x()+iconSize.width()+mParentLegend->iconTextPadding(), mRect.y(), textRect.width(), textHeight, Qt::TextDontClip, mPlottable->name());
//      }
//  } else {
...
//SAE
bool QCPLegend::addItemHor(QCPAbstractLegendItem *item)
{
    if (!hasItem(item))
    {
...
bool QCPLegend::addItemVer(QCPAbstractLegendItem *item)
{
    return addItem(item);
}
//SAE
...
//  painter->drawRect(mOuterRect);
  //SAE
  //NOTE: added because left side of outerRect wasn't drawn if legend is situated on the right
  auto rect = mOuterRect;
  rect.setWidth(rect.width()-2);
//  painter->drawRect(rect);
//  painter->drawRoundedRect(rect, QCP::gLegendFrameCornerRadius, QCP::gLegendFrameCornerRadius);
  QPen pen = painter->pen();
  pen.setColor(mTextColor);
  painter->setPen(pen);
  utility::drawRoundedRectWithShadow(*painter, rect, mLegendFrameCornerRadius, mLegendFrameCornerRadius, mIsLegendFrameShadowEnabled ? 4. : 0.);
...
//  QTextDocument td;
//  td.setTextWidth( mTextBoundingRect.width() );
//  td.setHtml("K<sub>max</sub>=K<sub>2</sub>");
//  drawHtmlText(painter, mRect, Qt::AlignCenter, td);
  //SAE
  ...
  if (QCP::gIsLatexMarkupEnabled) {
      QString newText;
      bool replacementsTookPlace;
      std::tie(newText, replacementsTookPlace) = latex2html(mText);
      if (replacementsTookPlace) {
          QTextDocument td;
          td.setTextWidth( mTextBoundingRect.width() );
          td.setHtml(newText);
...
QCPGraph::QCPGraph(QCPAxis *keyAxis, QCPAxis *valueAxis) :
  QCPAbstractPlottable(keyAxis, valueAxis), mBasisFillType(BasisFillType::ZeroBasis), mIsCustomColormapEnabled(false)
{
...
void QCPGraph::setFillBasis(QCPGraph::BasisFillType fillType)
{
    if (mBasisFillType != fillType) {
        mBasisFillType = fillType;
    }
}

void QCPGraph::setColorMapEvaluator(std::shared_ptr<ColorMapEvaluator> evaluator)
{
    mColorMapEvaluator = evaluator;
    if (evaluator)
        mIsCustomColormapEnabled = true;
    else
        mIsCustomColormapEnabled = false;
}

void QCPGraph::enableCustomColormap(bool enabled)
{
    mIsCustomColormapEnabled = enabled;
}
...
void QCPGraph::drawLegendIcon(QCPPainter *painter, const QRectF &rect) const
{
  ...
  if (!mScatterStyle.isNone())
  {
    bool dataLabelsEnabled = mScatterStyle.dataLabelsEnabled();
    const_cast<QCPScatterStyle&>(mScatterStyle).enableDataLabels(false);
    ...
    const_cast<QCPScatterStyle&>(mScatterStyle).enableDataLabels(dataLabelsEnabled);
  }
}
...
//    removeFillBasePoints(lineData);
    ...
    //SAE
    if (mBasisFillType != BasisFillType::TotalFill) {
 //     draw base fill under graph, fill goes all the way to the zero-value-line:
        addFillBasePoints(lineData);
        painter->setPen(Qt::NoPen);
        painter->setBrush(mainBrush());
        ...
        if (mData->size() != 0) {
            auto r = mKeyAxis->mAxisRect->rect();
            auto it1 = mData->constBegin();
            int left = mKeyAxis->coordToPixel(mData->constBegin()->key);
            int right = mKeyAxis->coordToPixel(std::prev(mData->constEnd())->key);
            for (; it1 != mData->constEnd(); ++it1) {
                if (!qIsNaN(it1->value)) {
                    auto it2 = it1;
                    while (it2 != mData->constEnd() && !qIsNaN(it2->value))
                           ++it2;
                    it2 = std::prev(it2);
                    painter->drawRect(std::max(mKeyAxis->coordToPixel(it1->key), double(left)), r.top(),
                                 std::min(mKeyAxis->coordToPixel(it2->key) - mKeyAxis->coordToPixel(it1->key), double(right - left)), r.height());
                    it1 = it2;
                }
            }
        }
...
void QCPGraph::drawScatterPlot(QCPPainter *painter, QVector<QCPData> *scatterData) const
{
  ...
  {
//    for (int i=0; i<scatterData->size(); ++i)
    auto decimation = mScatterStyle.decimation(); //SAE
    for (int i=0; i<scatterData->size(); i += decimation) // SAE
...
void QCPGraph::drawLinePlot(QCPPainter *painter, QVector<QPointF> *lineData) const
{
  ...
  if (mainPen().style() != Qt::NoPen && mainPen().color().alpha() != 0)
  {
    ...
    */
    ...
    //SAE
    QMap<double, QColor> mColorMap;
    if (mIsCustomColormapEnabled && mColorMapEvaluator) {
        for (auto data : mData->keys()) {
            double newX = mKeyAxis->coordToPixel(data);
            mColorMap[newX] = mColorMapEvaluator->getColorForPoint((*mData)[data].key, (*mData)[data].value);
        }
//        for (auto data : mColorMap.keys()) {
//            qDebug() << mColorMap[data];
//        }
    }
    //SAE
...
//    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
    //SAE
    if (((mIsCustomColormapEnabled && mColorMapEvaluator)|| mParentPlot->plottingHints().testFlag(QCP::phFastPolylines)) &&
    //SAE
...
//            painter->drawLine(lineData->at(i-1), lineData->at(i));
              ...
              if (mIsCustomColormapEnabled && mColorMapEvaluator) {
                  QPen oldPen = painter->pen();
                  QPen pen = painter->pen();
                  QLinearGradient grad(lineData->at(i-1), lineData->at(i));
                  grad.setColorAt(0, mColorMap[lineData->at(i-1).x()]);
                  grad.setColorAt(1, mColorMap[lineData->at(i).x()]);
              ...
              //DEBUG - try to do smoothing for graphs
//              double s_a, c_a;
//              if ((lineData->at(i).x() - lineData->at(i-1).x())  < 0.01) {
//                 s_a = 1.;
//                 c_a = 0.;
//              } else {
//                  double tg_a = (lineData->at(i).y() - lineData->at(i-1).y())
//                          / (lineData->at(i).x() - lineData->at(i-1).x());
//                  c_a = sqrt(1/(1+tg_a*tg_a));
//                  s_a = tg_a > 0 ? sqrt(1 - c_a*c_a) : -sqrt(1 - c_a*c_a);
//              }

//              double d = 20;
//              QPainterPath path;
//              path.moveTo(lineData->at(i-1) + QPoint(d*s_a, -d*c_a));
//              path.lineTo(lineData->at(i-1) + QPoint(-d*s_a, +d*c_a));
//              path.lineTo(lineData->at(i) + QPoint(-d*s_a, +d*c_a));
//              path.lineTo(lineData->at(i) + QPoint(d*s_a, -d*c_a));
//              path.lineTo(lineData->at(i-1) + QPoint(d*s_a, -d*c_a));

//              QLinearGradient gradient(0.5*(lineData->at(i) + lineData->at(i-1)) +QPoint(d*s_a, -d*c_a), 0.5*(lineData->at(i) + lineData->at(i-1)) +QPoint(-d*s_a, +d*c_a)); // diagonal gradient from top-left to bottom-right
//              gradient.setColorAt(0,   QColor(100,200,200, 0));
//              gradient.setColorAt(0.25, QColor(100,200,200, 200));
//              gradient.setColorAt(0.5, QColor(100,200,200, 255));
//              gradient.setColorAt(0.25, QColor(100,200,200, 200));
//              gradient.setColorAt(1,   QColor(100,200,200, 0));
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const
{
  ...
  if (valueAxis->scaleType() == QCPAxis::stLinear)
  {
    ...
    } else if (keyAxis->axisType() == QCPAxis::atBottom)
    {
      ...
      //SAE
      switch (mBasisFillType) {
          case (BasisFillType::ZeroBasis):
              point.setY(valueAxis->coordToPixel(0));
              break;
          case (BasisFillType::HighBasis):
              point.setY(keyAxis->axisRect()->top());
              break;
          case (BasisFillType::LowBasis):
              point.setY(keyAxis->axisRect()->bottom());
              break;
          case (BasisFillType::TotalFill):
              qFatal("this function was not supposed to be run for BasisFillType::TotalFill");
              break;
      }
      //SAE
    }
...
//  for (int i=0; i<pointData->size(); ++i)
  auto decimation = mScatterStyle.decimation(); //SAE
  for (int i=0; i<pointData->size(); i += decimation) //SAE
...
////              td.setTextWidth( mTextBoundingRect.width() );
//          td.setHtml(newText);
//          textRect = QRect(QPoint(0,0), sizeHint(painter, textRect, Qt::TextDontClip|mTextAlignment, td));
//      }
//  }
  //SAE
  QRect textBoxRect = textRect.adjusted(-mPadding.left(), -mPadding.top(), mPadding.right(), mPadding.bottom());
...
////              td.setTextWidth( mTextBoundingRect.width() );
//            td.setHtml(newText);
//            drawHtmlText(painter, textRect, Qt::TextDontClip|mTextAlignment, td);
//        } else {
//            painter->drawText(textRect, Qt::TextDontClip|mTextAlignment, mText);
//        }
//    } else {
...
void QCPItemTracer::setIterpolationType(QCPGraph::LineStyle newStyle)
{
    if (mInterpolationType != newStyle)
        mInterpolationType = newStyle;
}
...
void QCPItemTracer::draw(QCPPainter *painter)
{
  ...
  //SAE
//  qDebug() << "position.key" << position->key();
//  qDebug() << "position.value" << position->value();
//  qDebug() << "center.x" << center.x();
//  qDebug() << "center.y" << center.y();
//  qDebug() << "painter->viewport().left()" << painter->viewport().left();
//  qDebug() << "painter->viewport().top()" << painter->viewport().top();
//  qDebug() << "painter->viewport().right()" << painter->viewport().right();
//  qDebug() << "painter->viewport().bottom()" << painter->viewport().bottom();
  //SAE
  ...
  //SAE
//  qDebug() << "clip.left()" << clip.left();
//  qDebug() << "clip.top()" << clip.top();
//  qDebug() << "clip.right()" << clip.right();
//  qDebug() << "clip.bottom()" << clip.bottom();
  //SAE
...
//          position->setCoords(first.key(), first.value().value);
            //SAE
          position->setCoords(first.key(), NAN);
            //SAE
...
//              position->setCoords(mGraphKey, (mGraphKey-prevIt.key())*slope+prevIt.value().value);
              ...
              switch (mInterpolationType) {
                  ...
                  case QCPGraph::LineStyle::lsNone:
                      position->setCoords(mGraphKey, NAN);
                      break;
                  ...
                  case QCPGraph::LineStyle::lsStepLeft:
                      position->setCoords(mGraphKey, prevIt.value().value);
                      break;
                  case QCPGraph::LineStyle::lsStepRight:
                      position->setCoords(mGraphKey, it.value().value);
                      break;
                  case QCPGraph::LineStyle::lsStepCenter:
                      if (fabs(mGraphKey-prevIt.key()) < fabs(mGraphKey-it.key())) {
                        position->setCoords(mGraphKey, prevIt.value().value);
                      } else {
                        position->setCoords(mGraphKey, it.value().value);
                      }
